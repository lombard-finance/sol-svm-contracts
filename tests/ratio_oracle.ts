...
describe("Ratio Oracle", () => {
  ...
  describe("Ownership", function () {
    ...
    it("acceptOwnership: failure from unauthorized party", async () => {
      await expect(
        program.methods.acceptOwnership()
          .accounts({ 
  ...
  describe("Admin Functions", function () {
    it("updateConsortium: successful by admin", async () => {
      ...
      const newConsortiumConfigPDA = PublicKey.findProgramAddressSync([Buffer.from("consortium_config")], newConsortium.publicKey)[0];
