...
if (process.argv.indexOf("--help") > -1) {
  console.log(`Usage: PROGRAM_ID=<program_id> ANCHOR_PROVIDER_URL=<rpc_url> ANCHOR_WALLET=<wallet_path> yarn mintFromPayload <mint_payload> <recipient>

    Mints the LBTC contained in <mint_payload> to <recipient>.`);
  process.exit(0);
}
...
(async () => {
  try {
    ...
    const tx = await program.methods.mintFromPayload(payloadHash).accounts({
      payer,
      config: configPDA,
