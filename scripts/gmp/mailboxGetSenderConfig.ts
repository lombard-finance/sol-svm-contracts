...
if (process.argv.indexOf("--help") > -1) {
  console.log(`Usage: PROGRAM_ID=<program_id> ANCHOR_PROVIDER_URL=<rpc_url> ANCHOR_WALLET=<wallet_path> yarn gmp_mailboxGetSenderConfig <sender authority address>
...
const senderAuthority = new PublicKey(process.argv[2]);
...
(async () => {
  try {
    ...
    console.log(`Mailbox sender config PDA for ${senderAuthority}: ${senderConfigPDA.toBase58()}`);

    const config = await program.account.senderConfig.fetch(senderConfigPDA);
    console.log(`sender config contents: ${JSON.stringify(config)}`)
  } catch (err) {
    console.error("Error getting sender config:", err);
  }
})();
