...
if (process.argv.indexOf("--help") > -1) {
  console.log(`Usage: PROGRAM_ID=<program_id> [MAILBOX_PROGRAM_ID=<mailbox_program_id>] ANCHOR_PROVIDER_URL=<rpc_url> ANCHOR_WALLET=<wallet_path> yarn gmp_assetRouterRedeem <source mint> <amount> <to account> <solana chain ID> <ledger chainId>

    Redeems StakedLBTC to native token.`);
  process.exit(0);
}
...
const sourceMint = new PublicKey(process.argv[2]);
const amount = new anchor.BN(process.argv[3]);
const recipient = new PublicKey(process.argv[4]);
const solanaChainID = Buffer.from(process.argv[5], "hex");
const ledgerChainID = Buffer.from(process.argv[6], "hex");
...
(async () => {
  try {
    ...
    const tokenRoutePDA = getAssetRouterTokenLocalRoutePDA(programId, solanaChainID, sourceMint, assetRouterConfig.nativeMint);
    ...
    const srcMintAccountInfo = await provider.connection.getAccountInfo(sourceMint);
    if (!srcMintAccountInfo) {
      throw new Error(`mint account not found: ${assetRouterConfig.nativeMint.toBase58()}`);
    }
    const srcTokenProgram = srcMintAccountInfo.owner;

    const dstMintAccountInfo = await provider.connection.getAccountInfo(assetRouterConfig.nativeMint);
    if (!dstMintAccountInfo) {
      throw new Error(`mint account not found: ${assetRouterConfig.nativeMint.toBase58()}`);
    }
    const dstTokenProgram = dstMintAccountInfo.owner;

    const payerTA = await spl.getAssociatedTokenAddress(sourceMint, payer, false, srcTokenProgram);
    const treasuryTA = await spl.getAssociatedTokenAddress(sourceMint, assetRouterConfig.treasury, false, srcTokenProgram);
    const recipientTA = await spl.getAssociatedTokenAddress(assetRouterConfig.nativeMint, recipient, false, dstTokenProgram);

    console.log(`Payer Token Account: ${payerTA.toBase58()}`);

    const tx = await program.methods.redeem(
      Array.from(Uint8Array.from(solanaChainID)), Array.from(Uint8Array.from(assetRouterConfig.nativeMint.toBytes())), Array.from(Uint8Array.from(recipientTA.toBytes())), amount
    ).accounts({
      payer: payer,
      tokenRoute: tokenRoutePDA,
      payerTokenAccount: payerTA,
      tokenProgram: srcTokenProgram,
      mint: sourceMint,
    ...
    console.error("Error depositing native token:", err);
  }
})();
