...
if (process.argv.indexOf("--help") > -1) {
  console.log(`Usage: PROGRAM_ID=<program_id> ANCHOR_PROVIDER_URL=<rpc_url> ANCHOR_WALLET=<wallet_path> yarn gmp_assetRouterUnsetTokenRoute <admin> <from chain> <from token> <to chain> <to token> <route type>
...
let fromToken;
let toToken;
if (process.argv[7] == "deposit") {
  pathType = depositPathType;
  fromToken = Array.from(Uint8Array.from(Buffer.from(process.argv[4], "hex")));
  toToken = (new PublicKey(process.argv[6])).toBytes();
} else if (process.argv[7] == "deposit-local")  {
  pathType = depositPathType;
  fromToken = (new PublicKey(process.argv[4])).toBytes();
  toToken = (new PublicKey(process.argv[6])).toBytes();
} else if (process.argv[7] == "redeem")  {
  pathType = depositPathType;
  fromToken = (new PublicKey(process.argv[4])).toBytes();
  toToken = Array.from(Uint8Array.from(Buffer.from(process.argv[6], "hex")));
} else if (process.argv[7] == "redeem-local")  {
