...
import { MAILBOX_CONFIG_SEED } from "./constants";

// Provide instructions.
if (process.argv.indexOf("--help") > -1) {
  console.log(`Usage: PROGRAM_ID=<asset_router_program_id> ANCHOR_PROVIDER_URL=<rpc_url> ANCHOR_WALLET=<wallet_path> yarn gmp_mailboxUnpause <admin> [--populate]

    Unpauses Mailbox contract.`);
  process.exit(0);
}
...
(async () => {
  try {
    ...
    const tx = await program.methods.unpause().accounts({
      admin,
      config: configPDA
    });
