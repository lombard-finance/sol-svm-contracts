...
if (process.argv.indexOf("--help") > -1) {
  console.log(`Usage: PROGRAM_ID=<program_id> ANCHOR_PROVIDER_URL=<rpc_url> ANCHOR_WALLET=<wallet_path> yarn gmp_setInitialValidatorSet <admin> <valset_payload>

    Sets the initial validator set on the Consortium program.`);
  process.exit(0);
}
...
(async () => {
  try {
    ...
    const tx = await program.methods.setInitialValset(valsetPayload).accounts({
      admin: admin,
      config: configPDA,
    });
