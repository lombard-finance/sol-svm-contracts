...
if (process.argv.indexOf("--help") > -1) {
  console.log(`Usage: PROGRAM_ID=<program_id> [MAILBOX_PROGRAM_ID=<mailbox_program_id>] ANCHOR_PROVIDER_URL=<rpc_url> ANCHOR_WALLET=<wallet_path> yarn gmp_assetRouterDeposit <destination mint> <amount> <to account> <solana chain ID> <ledger chainId>
...
const destinationMint = new PublicKey(process.argv[2]);
...
(async () => {
  try {
    ...
    const srcMintAccountInfo = await provider.connection.getAccountInfo(assetRouterConfig.nativeMint);
    ...
    const dstMintAccountInfo = await provider.connection.getAccountInfo(destinationMint);
    if (!dstMintAccountInfo) {
      throw new Error(`mint account not found: ${assetRouterConfig.nativeMint.toBase58()}`);
    }
    const dstTokenProgram = dstMintAccountInfo.owner;

    const depositorTA = await spl.getAssociatedTokenAddress(assetRouterConfig.nativeMint, payer, false, srcTokenProgram);
    const recipientTA = await spl.getAssociatedTokenAddress(destinationMint, recipient, false, dstTokenProgram);

    console.log(`Depositor Token Account: ${depositorTA.toBase58()}`);
    console.log(`Recipient Token Account: ${recipientTA.toBase58()}`);

    const tx = await program.methods.deposit(
      Array.from(Uint8Array.from(solanaChainID)), Array.from(Uint8Array.from(destinationMint.toBytes())), Array.from(Uint8Array.from(recipientTA.toBytes())), amount
    ).accounts({
      payer: payer,
      tokenRoute: tokenRoutePDA,
      payerTokenAccount: depositorTA,
      tokenProgram: srcTokenProgram,
      mint: assetRouterConfig.nativeMint,
