...
if (process.argv.indexOf("--help") > -1) {
  console.log(`Usage: PROGRAM_ID=<program_id> ANCHOR_PROVIDER_URL=<rpc_url> ANCHOR_WALLET=<wallet_path> yarn gmp_initializeConsortium <admin address>
...
(async () => {
  try {
    ...
    const tx = await program.methods.initialize(admin).accounts({
      deployer: deployer,
