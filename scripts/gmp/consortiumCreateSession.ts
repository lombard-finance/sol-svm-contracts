...
import { convertToBuf, getConsortiumConfigPDA, getConsortiumSessionPDA, getConsortiumValidatedPayloadPDA } from "./utils";

// Provide instructions.
if (process.argv.indexOf("--help") > -1) {
  console.log(`Usage: PROGRAM_ID=<program_id> ANCHOR_PROVIDER_URL=<rpc_url> ANCHOR_WALLET=<wallet_path> yarn gmp_consortiumCreateSession <payload>

    Creates new session on Consortium contract. `);
  process.exit(0);
}
...
const payload = Array.from(Uint8Array.from(convertToBuf(process.argv[2])));
...
(async () => {
  try {
    ...
    const tx = await program.methods.createSession(payloadHash).accounts({
      payer: payer,
      session: sessionPDA,
