...
if (process.argv.indexOf("--help") > -1) {
  console.log(`Usage: PROGRAM_ID=<program_id> ANCHOR_PROVIDER_URL=<rpc_url> ANCHOR_WALLET=<wallet_path> yarn crosschain_tokenPoolGetStateConfig <mint address>
...
(async () => {
  try {
    ...
    console.log(`State config PDA: ${statePDA.toBase58()}`);

    const config = await program.account.state.fetch(statePDA);
