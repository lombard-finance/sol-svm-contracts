...
if (process.argv.indexOf("--help") > -1) {
  console.log(`Usage: PROGRAM_ID=<program_id> ANCHOR_PROVIDER_URL=<rpc_url> ANCHOR_WALLET=<wallet_path> yarn disableBascule

    Disables the bascule checks for minting instructions.`);
  process.exit(0);
}
...
(async () => {
  try {
    ...
    const tx = await program.methods.disableBascule().accounts({
      payer,
      config: configPDA
    });
