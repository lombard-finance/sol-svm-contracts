...
if (process.argv.indexOf("--help") > -1) {
  console.log(`Usage: PROGRAM_ID=<program_id> ANCHOR_PROVIDER_URL=<rpc_url> ANCHOR_WALLET=<wallet_path> yarn enableWithdrawals

    Enables the redeem functionality.`);
  process.exit(0);
}
...
(async () => {
  try {
    ...
    const tx = await program.methods.enableWithdrawals().accounts({
      payer,
      config: configPDA
    });
