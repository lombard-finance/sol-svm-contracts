...
if (process.argv.indexOf("--help") > -1) {
  console.log(`Usage: PROGRAM_ID=<program_id> ANCHOR_PROVIDER_URL=<rpc_url> ANCHOR_WALLET=<wallet_path> yarn setInitialValidatorSet <valset_payload>

    Sets the initial validator set on the LBTC program.`);
  process.exit(0);
}
...
(async () => {
  try {
    ...
    const tx = await program.methods.setInitialValset().accounts({
      payer,
      config: configPDA,
